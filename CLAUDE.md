...
**⛔ The CSV carries no instrument identity** — its header is only `Date, Open, High, Low, Close, Volume`. If `%21NAAIM` ever stops resolving and the endpoint serves any other well-formed daily series, the parse succeeds and the tile publishes a different instrument's number under the NAAIM label. The `NAAIM_RANGE` guard is the only leg that refuses that, the same job the range and sum checks do in `parse_aaii_sentiment`.
