...
- **⛔ Never push a floor onto a screener field that is not in metainfo — a field the scanner does not publish returns `null` for every row instead of erroring, and the filter then matches nothing.** This is the same silent shape as the missing `bid`/`ask` above, and it recurred: `Value.Traded` (session-to-date traded value) served values until 2026-08-20 and by 2026-08-21 it was gone from the 3,771-field metainfo and null on every row. Measured 09:34 ET with the market open and `close`/`volume` both live, `Value.Traded >= $1M` matched **0 of 13,661** rows while the average-volume leg alone matched 2,806 — so the equity universe was empty on every poll and the tab went dark for the whole session with no error anywhere. Today's traded value is derived from `close * volume` and floored after the fetch, alongside `min_avg_dollar_vol`. The `limit` now truncates before that floor, so `order_by` decides what is lost above the cap; it is session volume, not average volume, because a gapper on 20x its normal participation is exactly the row this tool must not drop.
