...
| `data/nasdaq_ad_history.json` | JSON | `{date: {advances, declines}}` — cached Nasdaq A/D counts (committed; refreshed on a 90-session trailing window each run so the multi-year history is paid for once) |
