...
**What is NOT measured, stated plainly.** No forward-return test of the new columns, as above. The floors themselves are judgments: 1.2 admitted 401 of 1,776 liquidity-filtered rows mid-session on 2026-09-16 (244 strong, 207 weak, 50 in both) and 1.5 admitted 276 of 1,824 after the close, but nothing shows those cutoffs select better tickers than neighbouring values. The breadth coefficient was chosen, not swept. **TradingView's own Relative Volume at Time study has never been read off a crypto chart**, so the UTC-midnight anchor is verified against the vendor's `volume` column but not against the vendor's own plot.

**The full-universe warm-up has now been run once, pre-market on 2026-09-18**, and the figure is bracketed rather than exact: the equity status pill still read `pending` about 3.5 minutes in and `ready (1801/1821)` about 7.5 minutes in, so one full run takes somewhere between those two and resolves **98.9%** of the requested symbols. It writes a 2.2 MB cache. That is slower than the ~1.7 minutes of the yfinance warm-up it replaces and slower than the 3–5 minutes projected from a 60-symbol list, so **launch pre-market rather than at the bell** — starting at 09:25 would leave the board dark through the open. The 20 unresolved symbols have no baseline, score 0 and stay off the board, which is the fail-closed rule working rather than a fault. Re-time it if `DEFAULT_WORKERS`, the bar count or the universe floors change.
