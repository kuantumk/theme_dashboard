...
**The breadth card's tile grid re-shapes on the card's width, not the viewport's.** Seven tiles in two rows of twelve columns: three sentiment tiles at 3 + 6 + 3, then four breadth percentages at 3 each. AAII holds span 6 — measured 267px, byte-identical to the half-width it had under the old two-column grid — because it is the one tile rendering three figures and has no room to give. Below a 530px card the grid falls back to two columns and the last tile stretches to close the row. **The threshold is a container query, not a media query, and that is load-bearing**: this panel is drag-resizable, so on a wide monitor a media query reports "wide" while the panel is narrow and hands seven tiles a box that fits two. 530px is measured, not chosen — a span-3 tile then has 89px of text room against "CNN Fear & Greed" at 87px, the widest string any narrow tile must hold. Re-measure before changing a label, a font size, or the gap.

**⛔ `.breadth-value` is retuned in place; the compact scale is never a modifier class on the value element.** `loadBreadthData` rewrites that element's `className` wholesale in two places (`'breadth-value ' + colorClass` for the four breadth tiles, and the Fear & Greed branch), so an extra class survives first paint and is wiped the instant the payload arrives — the numbers snap back to 28px inside a span-3 tile. The placeholder em-dash state still looks correct, so a by-eye check passes and only live data exposes it. `.aaii-num` gets away with its own class solely because the AAII branch writes `textContent` and never touches `className`.
