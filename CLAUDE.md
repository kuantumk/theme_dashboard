...
**⛔ `relative_volume_10d_calc` is not time-of-day adjusted, so a fixed floor on it is a different filter every hour.** It is session-to-date volume divided by the 10-day average **full-day** volume — verified, not assumed: across 59 liquid tickers the implied divisor (`volume / rvol`) matched the true 10-day average daily volume to a median error of **0.02%** (the 30-day average was off by 13.8%). Flooring the raw figure at 1.5 therefore demands **16.3x** normal participation at 09:35, 7.5x at 10:00, 4.8x at 10:30 and 1.8x at 15:00 — strictest exactly when a momentum trader needs it loosest. On 2026-08-14 the raw leg admitted **20 rows out of 2,191**, and FCEL ran +14% on twice normal participation from 10:00 without the volume leg ever firing. `src/bidask/volume_curve.py` divides by the expected share of the session's volume so the floor means one thing all day, and `bidask.in_play_min_volume_pace` (1.0 = normal pace for this hour) replaces `in_play_min_rvol`. The retired key **raises** rather than being ignored, because silently dropping it would disable the leg — the exact failure being fixed. The curve is a market-wide median (36 tickers x 5 sessions); `ep_scan_common.calculate_rvol_at_time` answers the same question per ticker against its own history, which is strictly better and costs one Alpaca request per ticker — affordable for ~30 earnings candidates, not for ~2,000 screener rows every few seconds.
