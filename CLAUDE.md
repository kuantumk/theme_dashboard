...
3. **Market Breadth** NCFD/MMFI scraped from barchart.com via Selenium → `data/market_breadth_latest.json` (the published `docs/data/market_breadth.json` is written later, by `update_breadth_history` in step 10, which also fetches CNN Fear & Greed and the AAII survey — see AAII below); then **step 3b** computes the Nasdaq McClellan Summation Index + RSI(14) → `docs/data/nasi.json` (see NASI below). Both are non-critical — a failure logs a warning and the workflow continues.
