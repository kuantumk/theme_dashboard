...
**The weight is a user dial, not an earned weight.** `coil: 0.05` is the argmax of a sweep at both H=5 and H=10, but the gain is noise (+0.0001 / +0.0005 against zero) and everything above it costs real IC — −0.002 at 0.10, −0.009 at 0.30, −0.018 at 0.50. Set it to 0 to rank on strength alone; the marker and the counts render either way. Re-run `backtest_radar.py --mode weights` (the `coil_10`/`coil_20`/`coil_30` ladder) before moving it.

**⛔ No coil weight surfaces the motivating case, so do not reach for the weight to fix a ranking miss.** Reconstructed against real point-in-time legs, the 2026-09-11 board reproduces production's Cybersecurity rank 11 exactly at `coil: 0`. Sweeping the weight moves it 11 → 10 → 9 → **8, where it plateaus**: rank 8 at 0.30 and still rank 8 at 2.00, forty times the shipped value and far past the point where the leg would dominate the composite. The reason is arithmetic, not tuning — Cybersecurity was **4 of 33 coiled (12.1%)**, below Healthcare (15.7%), Biotech (14.4%) and Oil & Gas (14.0%), all of which sat above it. Raising the weight lifts them too, so the gap never closes.
