...
- **⛔ Never push a floor onto a screener field that is not in metainfo — the scanner may resolve it anyway, return `null` for every row instead of erroring, and the filter then matches nothing.** This is the same silent shape as the missing `bid`/`ask` above. `Value.Traded` (session-to-date traded value) is the case that bit: it has **never** been in the 3,771-field metainfo, yet the scanner serves it — an unlisted alias, unsupported, populated only part of the day. Measured 2026-08-21 it was null for every row through pre-market and for at least the first four minutes of the session (`Value.Traded >= $1M` matched **0 of 13,661** at 09:34 ET with `current_session` reading `market` and `close`/`volume` live, while the average-volume leg alone matched 2,806), and matched **4,231** by 17:21 ET. So the equity universe was empty on every poll until mid-morning and the tab was dark through the open with no error anywhere. It was never withdrawn and this is not a vendor regression to wait out — the dependency was always unsound. Today's traded value is derived from `close * volume`, which is defined in every session state, and floored after the fetch alongside `min_avg_dollar_vol`. The `limit` now truncates before that floor, so `order_by` decides what is lost above the cap; it is session volume, not average volume, because a gapper on 20x its normal participation is exactly the row this tool must not drop.
- **An absent feed reading must not render as a claim about the vendor.** `feed=""` used to print *delayed feed*, so during the `Value.Traded` outage the pills accused a streaming vendor of a stale feed — which is what made the failure unreadable. An empty `feed` now prints *feed unknown*; `Payload.delayed` still counts an unknown as delayed, so the styling and the never-present-stale-prices-as-live rule are untouched, and only the wording stops overclaiming. **`_feed_mode` and `_market_status` read their values off the rows**, so a zero-row response has no reading to carry: hoisting them above the empty-frame return (both fetchers do) covers only the case where rows arrived and our own floors emptied them. The rowless case is covered by the wording, not by the hoist.
...
- **⛔ The quote rule reads the PREVIOUS poll's book, and the tick rule wins every disagreement.** Both halves are load-bearing and both were wrong until 2026-08-20. Comparing `last` against the *current* snapshot compares a trade against a book that has already reacted to it — Lee & Ready's lag rule exists to prevent exactly this, and at a 10-second cadence the misalignment is guaranteed. Measured over 13,821 live observations, the quote rule scored on its own: Spearman with the same-window mid return **−0.185**, and **−0.009** against an independent 1-minute series; against the previous poll's book, **+0.339**. Prints average 0.353 of the spread when the mid rose and 0.633 when it fell. Separately, gating the tick-rule override on `quote_moved` handed the frozen-book cohort (3.4% of observations, every one with a zero mid move) to the quote rule; those observations carry no direction and contribute only the ticker's habitual position in its own spread. Replayed end to end against independent bars, the two fixes together moved the board from +0.458/+0.187 to **+0.487/−0.013** on (direction / static-position bias). Do not "restore" either the current-book comparison or the gate.
- **What the quote rule measures when it is pointed at the wrong book is not flow at all.** Scored alone against the current snapshot it correlates **+0.896** with a purely static per-ticker attribute — the mean position of that ticker's prints inside its own spread — and **−0.049** with its actual drift. That attribute is independent of direction (r = −0.075 with drift) and holds at equal strength inside every drift tercile. This is why the defect was invisible for months: the mid band and the tick override route 67% of decisions to the tick rule, so a *short* board still looked right.
- **Counters cover a trailing window; they must never run cumulatively again.** Each poll contributes the directional signal plus a small constant per-ticker bias. The signal is bounded by the day's net move; the bias grows with the poll count, so given enough polls the bias wins. Measured on the live dashboard on 2026-08-20 (513 polls since 09:12): its own hit *increments* tracked price at Spearman +0.31 over 30s, +0.40 over a minute and +0.47 over three minutes, while its **cumulative** margin scored +0.18 over that same stretch and **+0.10** against the day's move — with the strong column averaging −1.03% since 09:30 against the weak column's −1.28%. As a control, the count of up-minutes minus down-minutes explained the same day's returns at **+0.71**, so the information was in the tape and the accumulator was losing it. `bidask.hit_window_minutes` (30) bounds the bias term; 0 restores the old behaviour. **Be precise about how much the window is carrying, because it is less than that history suggests**: most of the constant bias came from the quote rule reading the wrong book, and the `classify.py` fix removes it at the source — on a 55-minute tape the corrected board correlates +0.04 with a static position-in-spread at *every* window length including none. Swept against each window's own horizon, 5/10/15/20/30/45-minute and cumulative all scored +0.30 to +0.40, so **30 is not a measured optimum** — it is a density-and-intent choice (167 median hits per ticker, 462 of 505 tickers on the board, and a claim about now rather than about 09:12). Re-tune it by re-running that sweep, not by argument. What is NOT measured: six hours of accumulation under the corrected rule. The 2026-08-20 evidence covers 55 minutes. The window is measured in **seconds of tape, not polls** — the in-app cadence control retunes the poll rate at runtime, and a poll-counted window would silently change horizon with it. The `window-pill` in the status bar exists because the chips show raw counts: without it, a 30-minute figure reads as a whole session.
- **Raw `margin` is dwell-biased.** It counts polls, so a ticker in play for the whole window accumulates more observations than one admitted late into it. On the pre-window all-day state Spearman(total_hits, |margin|) = +0.61; over a uniform-dwell window it dropped to −0.15, which is what identified dwell as the cause. The trailing window bounds this too, but does not remove it. `imbalance` is the scale-free counterpart and is already computed.
- **Live head-to-head, 2026-08-20 11:36 ET, both builds polling the same market at the same moment** (old: 860 cumulative polls; new: 171 polls, 30-min window). Against the return over the last 30 minutes — the horizon the board claims — margin scored **+0.291 old, +0.534 new**, and column separation went +0.36pp to **+0.72pp**. Against the return since 09:30 the two were level (+0.247 old, +0.256 new) even though the new board had seen only the last half hour. Note the old board's score against the day's move ranged from +0.10 at 10:38 to +0.25 at 11:36; quote a range, not a single figure.
- **A superseded validation is worse than none.** This file previously recorded "Spearman(imbalance, same-window return) = +0.305, sign agreement 67.7%" and concluded: *when the board looks wrong, suspect the gate, the ranking or the cap before the CLNV logic.* That sentence sent the next reader away from the actual bug. The measurement was real but its target was contaminated: the tick rule decides most observations and is `sign(last − prior last)`, so correlating the output against a **last-to-last** return is partly correlating it with itself (+0.72 that way, +0.40 against a bounce-free mid return). Score against mid-to-mid returns or an independent price series, never last-to-last.
