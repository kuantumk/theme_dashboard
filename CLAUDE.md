...
- **Composite (per ticker)** = 0.4·`rs_sts_pct` + 0.4·(VARS percentile across **all tagged tickers** in the master table, anchored before the price/liquidity floors — consistent with the fast leg, invariant to floor changes) + 0.2·`rela_perf_1mo_rank` — all legs 0-100, missing → 50. Raw VARS is never averaged with 0-100 legs here (scale mismatch)
