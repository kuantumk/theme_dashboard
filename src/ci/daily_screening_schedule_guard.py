"""Decide whether a dual-cron scheduled workflow should continue.

Legacy workflows can keep two UTC cron entries, one for PDT and one for PST.
This guard validates the cron expression that actually triggered the run
against the current Pacific UTC offset. That keeps those workflows correct
even if GitHub starts the job late.

The expected cron pair is read from environment variables so that workflows can
re-use the same guard:

    SCHEDULE_GUARD_PDT_CRON   (default: "30 20 * * 1-5" -> 1:30 PM PDT)
    SCHEDULE_GUARD_PST_CRON   (default: "30 21 * * 1-5" -> 1:30 PM PST)
"""
...
PDT_CRON = os.environ.get("SCHEDULE_GUARD_PDT_CRON", "30 20 * * 1-5")
PST_CRON = os.environ.get("SCHEDULE_GUARD_PST_CRON", "30 21 * * 1-5")
