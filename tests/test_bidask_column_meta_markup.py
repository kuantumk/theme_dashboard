...
# The window pill was pinned here too: the chips showed raw hit counts over a
# trailing window, and without the horizon on screen a 30-minute figure read as
# a whole session. Both are gone — the board is stateless and accumulates
# nothing — so the server no longer publishes `hit_window_minutes` and there is
# no horizon to state. Removing the pill's own markup is the rendering unit's.
