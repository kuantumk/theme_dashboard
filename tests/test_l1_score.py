...
from src.themes.ecosystem_score import (
...
class RollupTests(unittest.TestCase):
    ...
    def test_single_leaf_family_gets_no_boost(self):
        leaves = [
            make_leaf('AI / Lonely', 'AI', 90.0),
            make_leaf('Biotech / A', 'Biotech', 40.0),
            make_leaf('Biotech / B', 'Biotech', 45.0),
        ]
        out = rollup_ecosystems(leaves, CFG)
        ai = next(e for e in out['ecosystems'] if e['name'] == 'AI')
    ...
    def test_boost_matches_decoded_screenshot_relationships(self):
        """Locks the math decoded from the competitor screenshot:
        eco_raw = mean(top-K leaf raws); every family leaf's boosted - raw is
        the same beta * eco_raw; boosted_eco = eco_raw * (1 + beta)."""
        # 6 cyber leaves (like the screenshot) + filler families so cyber z > 0
        ...
        out = rollup_ecosystems(cyber + filler, CFG)
        eco = next(e for e in out['ecosystems'] if e['name'] == 'Cybersecurity')

        top5 = sorted((lf['raw'] for lf in eco['leaves']), reverse=True)[:5]
        self.assertAlmostEqual(eco['raw'], float(np.mean(top5)))
        self.assertGreater(eco['raw'], 0)
        expected_boost = 0.3 * eco['raw']
        self.assertAlmostEqual(eco['delta'], expected_boost)
        self.assertAlmostEqual(eco['boosted'], eco['raw'] * 1.3)
        for lf in eco['leaves']:
            self.assertAlmostEqual(lf['boosted'] - lf['raw'], expected_boost)

    def test_confirmed_family_outranks_equal_isolated_leaf(self):
        family = [make_leaf(f'Cybersecurity / L{i}', 'Cybersecurity', 80.0 + i * 0.1)
                  for i in range(3)]
        isolated = make_leaf('Space / Launch', 'Space', 80.2)
        filler = [make_leaf(f'Biotech / F{i}', 'Biotech', 40.0 + i) for i in range(8)]
        out = rollup_ecosystems(family + [isolated] + filler, CFG)

        cyber_ranks = [lf['global_rank'] for lf in family]
        self.assertGreater(isolated['global_rank'], max(cyber_ranks))

        ranks = sorted(lf['global_rank'] for lf in family + [isolated] + filler)
        self.assertEqual(ranks, list(range(1, len(ranks) + 1)))

        eco_ranks = [e['rank'] for e in out['ecosystems']]
        self.assertEqual(eco_ranks, list(range(1, len(eco_ranks) + 1)))
        boosted = [e['boosted'] for e in out['ecosystems']]
...
class ComputeRadarTests(unittest.TestCase):
    def test_end_to_end_schema_and_is_screened(self):
        ...
        ecos = snap['ecosystems']
        self.assertEqual([e['rank'] for e in ecos], [1, 2])
        self.assertEqual(ecos[0]['name'], 'Cybersecurity')
        self.assertEqual(ecos[0]['n_leaves'], 2)
        self.assertEqual(ecos[0]['n_members'], 4)
        self.assertEqual(ecos[0]['n_screened'], 2)
        # Two-leaf family gets boosted above its raw (positive eco_raw)
        self.assertGreater(ecos[0]['boosted'], ecos[0]['raw'])
