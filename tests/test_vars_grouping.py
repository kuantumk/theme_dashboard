...
THEME_MAP = {
    ...
    # Dual-tagged across L1s: the Software & Internet L1 (n=1) is
    # dropped by the L1 minimum, the cyber leaf keeps DDOG.
    'DDOG': ['Software & Internet / DevOps & Data', 'Cybersecurity / Data Security'],
    # Dual-tagged within ONE L1: must count once in L1 n / avg_rs.
    ...
    # Two-member L1: below the 3-member L1 minimum, must not render.
    'RKLB': ['Space / Launch'],
...
class VarsL1GroupingTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def _l1_section(self, name):
        return next(f for f in self.snap['themes'] if f['name'] == name)

    def test_only_l1s_meeting_minimum_render(self):
        names = [f['name'] for f in self.snap['themes']]
        self.assertEqual(sorted(names), ['AI', 'Cybersecurity'])
        # Space (n=2) and Software & Internet (n=1) fail the L1 min;
        # Uncategorized/Singleton are hidden outright.

    def test_l1_ranking_score_is_top5_pooled_vars(self):
        ai = self._l1_section('AI')
        cyber = self._l1_section('Cybersecurity')
        ...
        # Pure-VARS ranking keeps the stale-but-higher L1 first; rotation
    ...
    def test_two_member_leaf_renders_inside_l1(self):
        cyber = self._l1_section('Cybersecurity')
        ...
        # Leaves sort by avg VARS desc within the L1.
        leaf_avgs = [lf['avg_vars'] for lf in cyber['leaves']]
        self.assertEqual(leaf_avgs, sorted(leaf_avgs, reverse=True))
        # Every leaf resolves to its L1 hub.
        for grp in self.snap['themes']:
            for leaf in grp['leaves']:
                self.assertEqual(leaf['l1'], grp['name'])

    def test_dual_tagged_ticker_counts_once_per_l1(self):
        cyber = self._l1_section('Cybersecurity')
    ...
    def test_network_stays_leaf_level_for_rendered_l1s_only(self):
